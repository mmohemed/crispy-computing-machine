...
(function () {
  ...
  /* ══════════════ مستمعات تُربط مرة واحدة ══════════════ */

  /* ---------- نسخ الكود (بالتفويض — يعمل مع أي محتوى يُضاف لاحقاً) ---------- */
  ...
  /* ---------- حالة الصفحة الحالية (تُحدَّث عند كل تهيئة) ---------- */
  var frames = [];
  var readBar = null;
  ...
  function initLesson(root) {
    ...
    var tocLinks = Array.prototype.slice.call(root.querySelectorAll('.toc a[href^="#"]'));
    ...
    /* ---------- شريط تقدّم القراءة ---------- */
    readBar = root.querySelector('.read-progress');
    updateReadBar();
    ...
    /* ---------- تعليم الدرس كمكتمل ---------- */
    var markBtn = root.querySelector('.mark-done');
    ...
    /* ---------- إطارات العرض الحي ---------- */
    frames = Array.prototype.slice.call(root.querySelectorAll('.demo iframe'));
  }

  /* ---------- اربط نفسك بسلسلة التهيئة العامة ---------- */
  var previous = CW.initPage;
  CW.initPage = function (root) {
    if (previous) previous(root);
    initLesson(root);
  };

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function () { initLesson(); });
  } else {
    initLesson();
  }
})();
